...
def solve_dispatch(
    ...
    """Solve the day-ahead dispatch as an LP or MILP.

    The LP omits charge/discharge mutex constraints. The MILP adds a binary
    charge-mode variable to enforce physical mutual exclusivity.
    """
    n_time = len(df)
    ...
    if method == "milp":
        solver_info["mip_gap"] = float(solved["mip_gap"])
        solver_info["mip_node_count"] = int(solved["mip_node_count"])
    else:
        solver_info["iterations"] = int(solved["iterations"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="C题第一问LP/MILP储能调度求解程序")
    parser.add_argument("--input", type=Path, default=DEFAULT_INPUT)
    parser.add_argument("--template", type=Path, default=DEFAULT_TEMPLATE)
    parser.add_argument("--method", choices=("lp", "milp"), default="milp", help="求解模型，默认milp")
    ...
    output_path = args.output or OUTPUT_DIR / f"result1_{args.method}.xlsx"
    detail_path = args.detail or OUTPUT_DIR / f"question_one_detail_{args.method}.csv"

    df = load_day_data(args.input)
    result, solver_info = solve_dispatch(
        df, args.method, args.mip_gap, args.time_limit, args.solver_python
    )
    checks = validate_solution(result)

    detail_path.parent.mkdir(parents=True, exist_ok=True)
    result.to_csv(detail_path, index=False, encoding="utf-8-sig")
    fill_template(args.template, output_path, result)
