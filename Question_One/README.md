...
完整数学模型见 [model_One.md](model_One.md)，其中区分正式纯费用 MILP、可选字典序优化、已完成的灵敏度分析与尚未实现的鲁棒扩展。本目录由 `Question_One_optimized` 更名为 `Question_One`，内容为正式优化版；整理前的旧版目录已移除。仍需保留同级 `C题/附件` 中的原始输入和模板。

## 文件

| 文件 | 作用 |
| --- | --- |
| `model_One.md` | 与当前程序对应的完整数学模型、求解与结果说明 |
| `solve_question_one.py` | 参数化主程序，支持 LP、MILP 和字典序优化 |
| `solver_worker.py` | LP/MILP 求解模块 |
| `sensitivity_analysis.py` | 批量灵敏度分析 |
| `requirements.txt` | Python 依赖 |
| `run_lp.ps1` | 运行标准 LP |
| `run_milp.ps1` | 运行正式纯费用 MILP |
...
```powershell
python -m pip install -r Question_One\requirements.txt
```
...
python Question_One\solve_question_one.py `
  --method lp `
  --output Question_One\output\result1_lp.xlsx `
  --detail Question_One\output\question_one_detail_lp.csv
```
...
python Question_One\solve_question_one.py `
  --method milp `
  --objective-mode cost `
  --output Question_One\output\result1_milp.xlsx `
  --detail Question_One\output\question_one_detail_milp.csv
```
...
```powershell
python Question_One\sensitivity_analysis.py `
  --output-dir Question_One\output\sensitivity
```
