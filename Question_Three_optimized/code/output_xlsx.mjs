...
const inspection = [];
for (const [sheet, range] of [[planSheet, "EP1:EQ4"],
                              [adjustedSheet, "EP1:EQ4"],
                              [storageSheet, "A1:F7"],
                              [emergencySheet, "A1:C8"]]) {
  await workbook.inspect({kind: "table", range: `'${sheet.name}'!${range}`,
    include: "values,formulas", tableMaxRows: 8, tableMaxCols: 6, maxChars: 2000});
  // Some imported templates expand inspect() to a whole-workbook summary.
  // Persist only the explicitly requested cells, not that unbounded summary.
  inspection.push(JSON.stringify({sheet: sheet.name, range,
    values: sheet.getRange(range).values, formulas: sheet.getRange(range).formulas}));
}
const formulaErrors = await workbook.inspect({kind: "match",
  searchTerm: "#REF!|#DIV/0!|#VALUE!|#NAME\\?|#NUM!|#NULL!|#SPILL!|#CALC!",
  options: {useRegex: true, maxResults: 30}, maxChars: 1500});
for (const line of formulaErrors.ndjson.split("\n")) {
  if (!line.trim()) continue;
  const entry = JSON.parse(line);
  if (entry.kind === "notice" || entry.kind === "match") {
    inspection.push(JSON.stringify(entry));
  }
}
await fs.writeFile(`${outputPath}.inspect.ndjson`, inspection.join("\n"));
