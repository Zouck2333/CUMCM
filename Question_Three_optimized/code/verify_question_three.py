...
def independent_calendar_baseline(data: Any, target: int) -> np.ndarray:
    """Replay the fixed specification using augmented least squares."""
    if target == 0:
        return np.zeros(N_TIME)
    if target < 14:
        return data.load_kwh[max(0, target - 7)].copy()
    indices = list(range(max(0, target - 28), target))
    design = np.array([
        [1., (h-target)/28., ((h-target)/28.)**2]
        + [float(data.dates[h].weekday() == j) for j in range(6)]
        for h in indices
    ])
    query = np.array([1., 0., 0.] + [float(data.dates[target].weekday() == j)
                                    for j in range(6)])
    augmented = np.vstack((design, np.diag([1e-4] + [.1]*8)))
    observed = np.vstack((np.log(np.maximum(1., data.load_kwh[indices])),
                          np.zeros((9, N_TIME))))
    beta = np.linalg.lstsq(augmented, observed, rcond=None)[0]
    return np.exp(query @ beta)
...
def read_checkpoint(path: Path, label: str, data: Any, fingerprint: str) -> list[dict[str, Any]]:
    ...
    require(first_config.get("model_version") == "four_stage_causal_mpc_calendar_v5",
            f"{label}: unexpected model version")
    require(
        first_config.get("terminal_mode") == "free"
        and first_config.get("objective_mode") in {"primary", "lexicographic"}
        ...
        and first_config.get("forecast_mode") == "calendar"
        and first_config.get("forecast_window") == 28
        and first_config.get("forecast_degree") == 2,
        f"{label}: checkpoint is not the official three-scenario free-SOC model",
    )
    for i, row in enumerate(rows):
        wanted = ALL_FIRST + timedelta(days=i)
        baseline = independent_calendar_baseline(data, i)
        ...
        for j, stage in enumerate(row["stages"]):
            tau = 36*j
            bias = (float(np.mean(data.load_kwh[i, tau-36:tau]
                                  - baseline[tau-36:tau])) if j else 0.)
            expected_load = np.maximum(0., baseline[tau:] + bias)
            expected_pv = np.repeat(data.pv_forecast_kw[i, j], 6)[:N_TIME-tau]/6.
            for field, expected in (("load_forecast_kwh", expected_load),
                                    ("pv_forecast_kwh", expected_pv)):
                saved = np.asarray(stage.get(field, []), dtype=float)
                require(saved.shape == expected.shape and np.all(np.isfinite(saved)),
                        f"{label} {wanted} stage {j}: invalid {field}")
                require(float(np.max(np.abs(saved-expected))) < 1e-6,
                        f"{label} {wanted} stage {j}: noncausal/inconsistent {field}")
            ...
            lexicographic = first_config["objective_mode"] == "lexicographic"
            expected_keys = {"cost_risk", "throughput", "peak"} if lexicographic else {"cost_risk"}
            ...
            require((not lexicographic and tol1 is None and tol2 is None) or (
                    lexicographic and tol1 is not None and tol2 is not None
...
def verify_summaries(output: Path, rolling: list[dict[str, Any]],
    ...
    verify_pair(comparison["full_year"], (rolling, zero_only), "full year")
    for label, records in (("rolling", rolling), ("zero_only", zero_only)):
        for key, expected in (("initial_soc_kwh", records[0]["soc_start"]),
                              ("evaluation_initial_soc_kwh", records[31]["soc_start"]),
                              ("final_soc_kwh", records[-1]["soc_end"])):
            close(comparison["boundary_soc"][label][key], expected, f"{label} {key}")
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path(__file__).resolve().parents[1] / "output_optimized")
    ...
    report = {"status": "passed", "evaluation_start": FIRST.isoformat(),
              ...
              "detail_rows": N_DAYS * N_TIME,
              "forecast_replay": "passed: independently reconstructed all 1825 issued forecasts",
