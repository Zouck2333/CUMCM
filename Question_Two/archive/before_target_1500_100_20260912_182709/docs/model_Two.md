...
> 2026-09-12起正式默认程序和result2.xlsx采用 [净负荷风险校准模型](model_Two_calibrated.md)。本文件保留此前 `legacy_scenarios` 策略的公式作为历史参考，其中90%储备、PV弃光上界和正式期情景目标不再描述当前默认结果。新程序若需复现本策略，应显式设置 `--purchase-strategy legacy_scenarios --reserve-quantile 0.9`。
