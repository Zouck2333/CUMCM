"""One causal, remaining-day MILP for Question Four.
...
actual future load, PV, and prices are deliberately absent from its interface.
"""
...
def solve_stage(
    ...
    *,
    ...
    terminal_credit_per_soc: float = 0.0,
    ...
    if not math.isfinite(terminal_credit_per_soc) or terminal_credit_per_soc < 0:
        raise ValueError("terminal_credit_per_soc must be finite and nonnegative")
    if objective_mode not in {"primary", "lexicographic"}:
    ...
    if initial_grid is not None:
        adjustment_up_bound = np.maximum(0.0, grid_upper - initial_grid)
        upper_bounds[up_idx] = adjustment_up_bound
        upper_bounds[down_idx] = initial_grid
    ...
    if initial_grid is not None:
        for t in range(n):
            ...
            # If up and down are both positive, subtracting their minimum
            # preserves G-G0 and strictly lowers every scenario's cost.
            # Positive adjustment prices make sign binaries unnecessary;
            # the projected feasible decisions under upper cost locks agree.

    plan_constant = float(np.dot(price, initial_grid)) if initial_grid is not None else 0.0
    objective_constant = plan_constant + terminal_credit_per_soc * SOC_MIN
    ...
    primary_coeff[soc_idx[-1]] = -terminal_credit_per_soc
    for s in range(n_scenarios):
        for t in range(n):
            rows.add(
                [
    ...
    first_value = float(primary_coeff @ first.x + objective_constant)
    result = first
    ...
    if objective_mode == "lexicographic":
        ...
        second_hi = np.append(row_upper, first_value - objective_constant + tolerance_1)
        second = run(
    ...
    # A successfully solved MILP/LP may still contain a small negative flow
    # accepted by the default feasibility tolerance. Do not clip a material
    # flow and thereby break SOC. This branch is entered only for trajectories
    # that the preceding implementation rejected, preserving accepted runs.
    flow_indices = np.concatenate([grid_idx, charge_idx, discharge_idx])
    if float(np.min(np.asarray(result.x)[flow_indices])) < -1e-7:
        repair_started = time.perf_counter()
        fixed_lo = lower_bounds.copy()
        fixed_hi = upper_bounds.copy()
        modes = np.rint(np.asarray(result.x)[mode_idx]).astype(int)
        fixed_lo[mode_idx] = modes
        fixed_hi[mode_idx] = modes
        fixed_hi[charge_idx[modes == 0]] = 0.0
        fixed_hi[discharge_idx[modes == 1]] = 0.0
        equality = np.isfinite(final_lower) & (final_lower == final_upper)
        upper = np.isfinite(final_upper) & ~equality
        lower = np.isfinite(final_lower) & ~equality
        inequality_matrix = vstack([final_matrix[upper], -final_matrix[lower]], format='csr')
        inequality_rhs = np.concatenate([final_upper[upper], -final_lower[lower]])
        repaired = linprog(
            final_objective, A_ub=inequality_matrix, b_ub=inequality_rhs,
            A_eq=final_matrix[equality], b_eq=final_lower[equality],
            bounds=np.column_stack([fixed_lo, fixed_hi]), method='highs-ds',
            options={'primal_feasibility_tolerance': 1e-9,
                     'dual_feasibility_tolerance': 1e-9},
        )
        if not repaired.success:
            raise RuntimeError(f'high-precision fixed-binary LP failed: {repaired.message}')
        result = repaired
        activity = final_matrix @ repaired.x
        if max(float(np.max(final_lower - activity, initial=0.0)),
               float(np.max(activity - final_upper, initial=0.0)),
               float(np.max(fixed_lo - repaired.x, initial=0.0)),
               float(np.max(repaired.x - fixed_hi, initial=0.0))) > 1e-6:
            raise RuntimeError('high-precision LP violates a physical constraint or objective lock')
        name = 'high_precision_polish'
        stage_gaps[name] = 0.0
        stage_bounds[name] = float(repaired.fun)
        stage_objectives[name] = float(repaired.fun)
        stage_solve_seconds[name] = time.perf_counter() - repair_started
        stage_status[name] = f'status=0; {repaired.message}'
    ...
    return StageResult(
        ...
        primary_value=float(primary_coeff @ x + objective_constant),
