// Question Four template writer. Runs beside a junction to the bundled runtime.
...
if (previewOnly) {
  const workbook = await SpreadsheetFile.importXlsx(await FileBlob.load(process.argv[3]));
  await fs.mkdir(process.argv[4], {recursive:true});
  for (const sheet of workbook.worksheets.items) {
    const range = sheet.name.includes("充放") ? "A1:F14" : sheet.name.includes("紧急") ? "A1:C12" : "A1:H6";
    const preview = await workbook.render({sheetName:sheet.name,range,scale:1.5,format:"png"});
    await fs.writeFile(path.join(process.argv[4],`${sheet.name}.png`),new Uint8Array(await preview.arrayBuffer()));
  }
  console.log((await workbook.inspect({kind:"workbook,sheet",maxChars:1800,tableMaxRows:2,tableMaxCols:3})).ndjson);
  console.log(`Q4_PREVIEWS_WRITTEN:${workbook.worksheets.items.length}`);
} else {
const [sourcePath, templatePath, outputPath, strategy] = process.argv.slice(2);
if (!sourcePath || !templatePath || !outputPath || !["4-2", "4-3"].includes(strategy)) {
  throw new Error("Usage: output_xlsx.mjs days.json template.xlsx output.xlsx 4-2|4-3");
}
...
if (workbook.worksheets.items.length !== (strategy === "4-2" ? 3 : 4)) {
  throw new Error("The template has an unexpected number of worksheets");
}
const planSheet = workbook.worksheets.getItemAt(0);
const adjustedSheet = strategy === "4-3" ? workbook.worksheets.getItemAt(1) : null;
const storageSheet = workbook.worksheets.getItemAt(strategy === "4-3" ? 2 : 1);
const emergencySheet = workbook.worksheets.getItemAt(strategy === "4-3" ? 3 : 2);
...
const purchaseSheets = adjustedSheet ? [planSheet, adjustedSheet] : [planSheet];
...
for (const sheet of workbook.worksheets.items) {
  const range=sheet.name.includes("充放") ? "A1:F14" : sheet.name.includes("紧急") ? "A1:C12" : "A1:H6";
  checks.push((await workbook.inspect({kind:"table",range:`'${sheet.name}'!${range}`,include:"values,formulas",tableMaxRows:3,tableMaxCols:4,maxChars:1500})).ndjson);
}
checks.push((await workbook.inspect({kind:"match",searchTerm:"#REF!|#DIV/0!|#VALUE!|#NAME\\?|#N/A|#NUM!|#NULL!|#SPILL!|#CALC!",options:{useRegex:true,maxResults:30},maxChars:1500})).ndjson);
await fs.writeFile(`${outputPath}.inspect.ndjson`,checks.join("\n"));
