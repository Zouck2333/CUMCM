...
def _check_storage(sheet, records: list[dict]) -> None:
    ...
    for i, (row, record) in enumerate(zip(rows, (
        ...
        if block == 0:
            ...
            if row[4] != "0:00":
                raise AssertionError("wrong opening SOC time label")
        elif block == 1:
            _equal(row[5], record["soc_end"], f"storage day {day_index} closing SOC")
            if row[4] != "24:00":
                raise AssertionError("wrong closing SOC time label")
        elif row[4] is not None or row[5] is not None:
            raise AssertionError("unexpected extra SOC values")
        if block > 0 and row[0] is not None:
            raise AssertionError("repeated storage date")
        start = 24 * block
        end = start + 24
        if row[1] != f"{_clock(start)}-{_clock(end)}":
            raise AssertionError("wrong four-hour storage interval")
