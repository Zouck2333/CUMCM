...
def _record(result: DayResult, key: str) -> dict:
    ...
    return {
        ...
        "stage_status": [stage.solution.stage_status for stage in result.stages],
        "price_models": [stage.forecast.price_model for stage in result.stages],
        "load_forecast_at_execution": (
            result.stages[0].forecast.load_hat.tolist() if result.strategy == "4-2"
            else np.concatenate([s.forecast.load_hat[:36] for s in result.stages]).tolist()
        ),
        "pv_forecast_at_execution": (
            result.stages[0].forecast.pv_hat.tolist() if result.strategy == "4-2"
            else np.concatenate([s.forecast.pv_hat[:36] for s in result.stages]).tolist()
        ),
    }
...
def _load_checkpoint(path: Path, inputs: LoadedInputs, strategy: str, key: str) -> list[dict]:
    if not path.is_file():
        return []
    compatible_key = None
    provenance_path = path.parent / "checkpoint_compatibility.json"
    if provenance_path.is_file():
        provenance = json.loads(provenance_path.read_text(encoding="utf-8")).get(strategy)
        if provenance and provenance.get("current_run_fingerprint") == key:
            if hashlib.sha256(path.read_bytes()).hexdigest() != provenance["checkpoint_sha256"]:
                raise ValueError("versioned checkpoint content hash mismatch")
            compatible_key = provenance["original_run_fingerprint"]
    records: list[dict] = []
    previous_soc = INITIAL_SOC
    with path.open("r", encoding="utf-8") as stream:
        for index, line in enumerate(stream):
            record = json.loads(line)
            if record.get("run_fingerprint") not in {key, compatible_key} or record.get("run_fingerprint") is None:
    ...
    if compatible_key is not None and len(records) != 365:
        raise ValueError("compatible prior-version reuse is restricted to complete verified annual runs")
    return records
...
def _run_one(
    ...
    *,
    ...
    summary["annual_total_cost_yuan"] = sum(r["total_cost"] for r in records)
    summary["formal_start_soc_kwh"] = formal_records[0]["soc_start"]
    summary["year_end_soc_kwh"] = records[-1]["soc_end"]
    summary["load_forecast_mae_kw"] = 6.0 * float(np.mean([
        np.abs(np.asarray(r["load_forecast_at_execution"]) - d.load_kwh)
        for r, d in zip(formal_records, formal_days, strict=True)
    ]))
    ordered = sorted((r["total_cost"] for r in formal_records), reverse=True)
    tail_mass = 0.1 * len(ordered)
    full = int(math.floor(tail_mass))
    summary["daily_cost_cvar_90_yuan"] = float(
        (sum(ordered[:full]) + (tail_mass - full) * ordered[full]) / tail_mass
    )
    summary["run_fingerprint"] = key
    summary["record_generation_fingerprint"] = records[0]["run_fingerprint"]
...
def main() -> None:
    ...
    parser.add_argument("--load-mode", choices=("legacy", "calendar"), default="calendar")
    parser.add_argument("--price-mode", choices=("legacy", "calendar", "calendar_decay", "adaptive"), default="adaptive")
    parser.add_argument("--terminal-value-weight", type=float, default=0.0)
    parser.add_argument("--risk-weight", type=float, default=0.1)
    parser.add_argument("--objective-mode", choices=("primary", "lexicographic"), default="lexicographic")
    ...
    config = ModelConfig(load_mode=args.load_mode, price_mode=args.price_mode,
                         terminal_value_weight=args.terminal_value_weight,
                         risk_weight=args.risk_weight, objective_mode=args.objective_mode)
