...
from .forecasting import (
    PRICE_CANDIDATES, calendar_baseline, choose_price_candidate, price_candidates,
)
...
class ModelConfig:
    ...
    load_mode: str = "calendar"
    price_mode: str = "adaptive"
    ...
    def __post_init__(self) -> None:
        ...
        if self.load_mode not in {"legacy", "calendar"}:
            raise ValueError("load_mode must be legacy or calendar")
        if self.price_mode not in {"legacy", "calendar", "calendar_decay", "adaptive"}:
...
class HistoryModel:
    ...
    def __init__(self, config: ModelConfig = ModelConfig()) -> None:
        ...
        self.price_errors: list[list[np.ndarray]] = [[] for _ in range(N_STAGE)]
    ...
    def _baseline(self, target: date) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        if self.config.load_mode == "calendar":
            load = calendar_baseline(
                np.stack([h.load_kwh for h in self.days]),
                [h.day for h in self.days], target,
            )
    ...
    def _price_candidates(self, target: date, known: np.ndarray, stage: int,
                          legacy_base: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        if not self.days:
            legacy_base = np.full(N_TIME, known[0])
            calendar = legacy_base
        else:
            calendar = calendar_baseline(
                np.stack([h.price for h in self.days]), [h.day for h in self.days],
                target, degree=1, floor=PRICE_FLOOR,
            )
        return price_candidates(legacy_base, calendar, known, stage)

    def append(self, completed: DayData) -> None:
        self._check_next_day(completed.day)
        load_base, pv_base, price_base = self._baseline(completed.day)
        for stage in range(N_STAGE):
            ...
            predictions, _ = self._price_candidates(
                completed.day, completed.price[:tau + 1], stage, price_base,
            )
            self.price_errors[stage].append(
                np.mean(np.abs(predictions[:, 1:] - completed.price[tau + 1:]), axis=1)
            )
    ...
    def stage_forecast(
        ...
        *,
        ...
        forecasts, biases = self._price_candidates(target, known_price, stage, price_base)
        selected = (choose_price_candidate(self.price_errors[stage])
                    if self.config.price_mode == "adaptive"
                    else PRICE_CANDIDATES.index(self.config.price_mode))
        price = forecasts[selected]
        price_bias = float(biases[selected])
        price_model = PRICE_CANDIDATES[selected]
        ...
        if n == 0:
            return StageForecast(
                ...
                load_hat[None, :], pv_hat[None, :], np.array([1.0]), (), price_bias, price_model
            )
        if n < 3:
        ...
        return StageForecast(
            ...
            probabilities, tuple(self.days[h].day for h in representatives), price_bias, price_model
        )
...
def run_day(
    ...
    if (config.load_mode, config.price_mode) != (history.config.load_mode, history.config.price_mode):
        raise ValueError("history and dispatch forecast configurations differ")
