...
from causal_policy import PROTOCOL, protocol_hash, tune_policy
...
PURCHASE_QUANTILE = 0.85
FORECAST_METHOD = "calendar_trend"
LOAD_WINDOW_DAYS = 28
LOAD_TREND_DEGREE = 2
PV_WINDOW_DAYS = 14
RISK_GROUPING = "all"
...
def calibrate_purchase_margin(
    ...
    *,
    ...
    grouping: str = RISK_GROUPING,
) -> tuple[np.ndarray, list[int]]:
    """Recent joint net residuals; no current-day observations."""
    ...
    if grouping not in {"all", "legacy"}:
        raise ValueError(f"未知残差分组: {grouping}")
    selected = same_type if grouping == "legacy" and len(same_type) >= 5 else candidates
    if not selected:
...
def run_model(
    ...
    *,
    ...
    load_window_days: int = LOAD_WINDOW_DAYS,
    load_trend_degree: int = LOAD_TREND_DEGREE,
    pv_window_days: int = PV_WINDOW_DAYS,
    risk_grouping: str = RISK_GROUPING,
    parameter_mode: str = "causal_monthly",
    emergency_budget_yuan: float = 1000000.0,
    ...
    if forecast_method not in {"similar_day", "calendar_trend"}:
        raise ValueError(f"未知预测方法: {forecast_method}")
    if load_window_days < 14 or load_trend_degree not in (1, 2) or pv_window_days < 3:
        raise ValueError("无效的负荷或光伏趋势参数")
    if risk_grouping not in {"all", "legacy"}:
        raise ValueError(f"未知残差分组: {risk_grouping}")
    if parameter_mode not in {"fixed", "causal_monthly"}:
        raise ValueError(f"未知调参模式: {parameter_mode}")
    if parameter_mode == "causal_monthly" and (forecast_method != "calendar_trend" or purchase_strategy != "calibrated_quantile"):
        raise ValueError("历史滚动调参须使用calendar_trend预测及calibrated_quantile购电策略")
    if not math.isfinite(emergency_budget_yuan) or emergency_budget_yuan <= 0:
        raise ValueError("紧急费用预算必须为有限正数")
    ...
    causal_residual_load: list[np.ndarray] = []
    causal_residual_pv: list[np.ndarray] = []
    policy_date = ""

    for day_index in range(limit):
        current_date = dates[day_index]
        calendar_forecast = forecast_method == "calendar_trend" and current_date >= FORMAL_START
        causal_formal = parameter_mode == "causal_monthly" and current_date >= FORMAL_START
        if causal_formal and current_date.month != current_month:
            # Pass an actual-data prefix, not the full-year arrays. Selection
            # cannot access today's observations or any future actual values.
            spent = sum(float(r["emergency_cost_yuan"]) for r in daily_rows if not r["warmup"])
            chosen, update, causal_residual_load, causal_residual_pv = tune_policy(
                as_of=current_date, dates_history=dates[:day_index],
                actual_load_history=actual_load[:day_index], actual_pv_history=actual_pv[:day_index],
                prior_load=prior_load, prior_pv=prior_pv, price=price,
                eta_charge=eta_charge, eta_discharge=eta_discharge, delta_h=delta_h,
                reserve_mode=reserve_mode, quantile_method=quantile_method,
                mip_gap=mip_gap, time_limit=time_limit, objective_mode=objective_mode,
                emergency_spent=spent, emergency_budget=emergency_budget_yuan,
                formal_days_remaining=len(dates)-day_index,
                solve_day=solve_day_milp, compute_reserve=compute_reserve,
            )
            load_window_days, load_trend_degree, pv_window_days = (chosen[k] for k in
                ("load_window_days", "load_trend_degree", "pv_window_days"))
            risk_window_days, risk_radius_periods, purchase_quantile, reserve_quantile = (chosen[k] for k in
                ("risk_window_days", "risk_radius_periods", "purchase_quantile", "reserve_quantile"))
            risk_grouping = chosen["risk_grouping"]
            policy_date = current_date.isoformat()
            current_month = current_date.month
            policy_updates.append(update)
            monthly_parameters.append({"month":current_date.strftime("%Y-%m"),"warmup":False,
                "as_of_date":policy_date,"history_end":update["history_end"],**chosen})
            print(f"历史调参 {policy_date}，截止{update['history_end']}：{chosen}",flush=True)
        if current_date.month != current_month and not calendar_forecast:
            current_month = current_date.month
        ...
        elif calendar_forecast:
            # Preserve January warm-up and therefore the formal initial SOC.
            # Historical January residuals remain those actually issued then.
            similar_load_days = list(range(max(0, day_index - load_window_days), day_index))
            similar_pv_days = list(range(max(0, day_index - pv_window_days), day_index))
            predicted_load = calendar_load_forecast(
                actual_load[:day_index], dates, day_index, window=load_window_days, degree=load_trend_degree,
            )
            predicted_pv = recent_pv_forecast(actual_pv[:day_index], day_index, window=pv_window_days)
        else:
        ...
        calibration_load = causal_residual_load if causal_formal else residual_load
        calibration_pv = causal_residual_pv if causal_formal else residual_pv
        if calibrated:
            purchase_margin, risk_source_days = calibrate_purchase_margin(
                day_index, dates, calibration_load, calibration_pv,
                window_days=risk_window_days, radius_periods=risk_radius_periods,
                quantile=purchase_quantile, grouping=risk_grouping,
            )
        ...
        day_reserve_quantile = (PROTOCOL["warmup_reserve_quantile"]
            if parameter_mode == "causal_monthly" and not causal_formal else reserve_quantile)
        reserve = compute_reserve(
            calibration_load,
            calibration_pv,
            eta_discharge=eta_discharge,
            quantile=day_reserve_quantile,
            mode=reserve_mode,
        ...
        if causal_formal:
            causal_residual_load.append(load_error)
            causal_residual_pv.append(pv_error)
        ...
        daily_rows.append(
            {
                ...
                "forecast_method": "calendar_trend" if calendar_forecast else "similar_day",
                "parameter_mode": parameter_mode,
                "policy_as_of_date": policy_date,
                "load_window_days": load_window_days if calendar_forecast else "",
                "load_trend_degree": load_trend_degree if calendar_forecast else "",
                "pv_window_days": pv_window_days if calendar_forecast else "",
                "risk_window_days": risk_window_days if calibrated else "",
                "risk_radius_periods": risk_radius_periods if calibrated else "",
                "purchase_quantile": purchase_quantile if calibrated else "",
                "risk_grouping": risk_grouping if calibrated else "",
                "reserve_quantile": day_reserve_quantile,
                "k_load": current_theta.k_load,
                "rho_load": current_theta.rho_load,
                "k_pv": current_theta.k_pv,
                "rho_pv": current_theta.rho_pv,
    ...
    diagnostics = {
        ...
        "load_trend_degree": load_trend_degree,
        "pv_window_days": pv_window_days,
        "risk_grouping": risk_grouping,
        "forecast_warmup_method": "similar_day",
        "parameter_mode": parameter_mode,
        "emergency_budget_yuan": emergency_budget_yuan,
        "parameter_selection_scope": "January-only initialization, then monthly selection using completed historical days"
        if parameter_mode == "causal_monthly" else "fixed parameters; selection provenance depends on configuration",
        "risk_residual_method": "replayed_current_parameters" if parameter_mode == "causal_monthly" else "issued_forecasts",
        "tuning_protocol_sha256": protocol_hash() if parameter_mode == "causal_monthly" else None,
        ...
        "week_type_rule": "six learned weekday indicators with Sunday as reference; legacy workday split only in warm-up"
        if forecast_method == "calendar_trend" else "Monday through Friday are workdays; weekends are non-workdays",
    ...
    if parameter_mode == "causal_monthly":
        # A varying monthly parameter must never be reported as one annual value.
        for key in ("load_window_days", "load_trend_degree", "pv_window_days", "risk_window_days",
                    "risk_radius_periods", "purchase_quantile", "reserve_quantile"):
    ...
    solution_payload = {
        ...
        "tuning_protocol": PROTOCOL if parameter_mode == "causal_monthly" else None,
...
def main() -> None:
    ...
    parser.add_argument("--forecast-method", choices=("similar_day", "calendar_trend"), default=FORECAST_METHOD)
    parser.add_argument("--load-window-days", type=int, default=LOAD_WINDOW_DAYS)
    parser.add_argument("--load-trend-degree", type=int, choices=(1, 2), default=LOAD_TREND_DEGREE)
    parser.add_argument("--pv-window-days", type=int, default=PV_WINDOW_DAYS)
    parser.add_argument("--risk-grouping", choices=("all", "legacy"), default=RISK_GROUPING)
    parser.add_argument("--parameter-mode", choices=("fixed", "causal_monthly"), default="causal_monthly")
    parser.add_argument("--emergency-budget-yuan", type=float, default=1000000.)
    ...
    if args.parameter_mode == "causal_monthly":
        args.output_json.parent.mkdir(parents=True,exist_ok=True)
        (args.output_json.parent/"tuning_protocol.json").write_text(json.dumps({
            "protocol":PROTOCOL,"sha256":protocol_hash(),"frozen_before_full_horizon_run":True,
    ...
    solution, daily_rows, detail_rows = run_model(
        ...
        emergency_budget_yuan=args.emergency_budget_yuan,
    )
