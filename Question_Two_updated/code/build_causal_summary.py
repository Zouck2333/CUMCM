...
BASELINE = "reference/previous_fixed_summary.json"
...
def build_summary(out,baseline_path=None):
    checks,benchmark=analysis_checks(out)
    ...
    v=read(out/"verification_report.json")
    assert sha(out/"input_data.json")==old["input_sha256"],"前后输入文件发生变化"
    assert abs(solution["days"][0]["soc_start"]-old["formal_initial_soc_kwh"])<1e-6
    ...
    sources=[root/"data/附件1.xlsx",root/"data/附件2.xlsx",root/"data/result2_template.xlsx",baseline_path]
    ...
    sources += sorted((root/"code").glob("*.py")) + sorted((root/"code").glob("*.mjs"))
    sources += [root/"docs/model_Two.md",root/"run_question_two.ps1"]
    manifest={"parameter_mode":"causal_monthly","same_input_values":True,"same_physical_parameters":True,
        ...
        "baseline_summary":baseline_path.relative_to(root).as_posix(),"tuning_protocol_sha256":d["tuning_protocol_sha256"],
        ...
        "path_base":"package root","sha256":{p.relative_to(root).as_posix():sha(p) for p in sources}}
