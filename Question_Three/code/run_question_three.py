...
def historical_load_baseline(data: InputData, day_index: int) -> np.ndarray:
    if day_index == 0:
...
def _record_config(
    ...
    return {
        ...
        "model_version": "four_stage_causal_mpc_free_soc_v4",
...
def run_strategy(
    ...
    *,
    ...
    for day_index in range(len(records)):
        baseline = historical_load_baseline(data, day_index)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path(__file__).resolve().parents[1] / "output_free_soc"
    )
    parser.add_argument(
        "--strategy", choices=("rolling", "zero_only", "both"), default="both"
    )
    parser.add_argument(
        "--objective-mode", choices=("lexicographic", "primary"), default="lexicographic"
    )
    parser.add_argument(
    ...
    if args.max_days == 365:
        ...
        official_parameters = (
            ...
            and args.objective_mode == "lexicographic"
